...
export const metadata: Metadata = pageMetadata({
  ...
  description: "Upcoming events and gatherings (English) at Praise Tabernacle.",
  path: "/events/en",
})

export default async function EventsEnglishPage() {
  const base = events.filter((e) => e.language === "en" || e.language === "bilingual")
  ...
  const jsonLdItemList = {
    ...
    "@type": "ItemList",
    name: "Praise Tabernacle Events (English)",
  ...
  return (
    <>
      <Script
        id="schema-org-events-list-en"
        type="application/ld+json"
        strategy="beforeInteractive"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLdItemList) }}
      />
      <PageHeader
        titleEn="Events (English)"
        titleTa="நிகழ்வுகள் (ஆங்கிலம்)"
        descriptionEn="Prayer times, youth gatherings, family events, and community connections (English)."
        descriptionTa="ஜெப நேரங்கள், இளைஞர் கூடுகைகள், குடும்ப நிகழ்வுகள், சமூக இணைப்பு (ஆங்கிலம்)."
      />
      ...
      <section className="bg-white">
        <Container className="section-padding">
          <div className="mb-6 flex flex-wrap items-center gap-2">
            ...
            <Link href="/events/en" className="btn btn-sm btn-primary">
