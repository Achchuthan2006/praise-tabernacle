...
export async function POST(req: Request) {
  try {
    ...
    const rl = rateLimit({ key: `prayer:${ip}`, max: 5, windowMs: 60_000 })
    if (!rl.allowed) {
    ...
    if (name.length > 120) return Response.json({ ok: false, error: "invalid_name" }, { status: 400 })
    if (email && !isValidEmail(email)) return Response.json({ ok: false, error: "invalid_email" }, { status: 400 })
    if (!message || message.length > 5000)
      return Response.json({ ok: false, error: "invalid_message" }, { status: 400 })

    await appendSubmission("prayer", {
      kind: "prayer",
      name,
      email,
      message,
